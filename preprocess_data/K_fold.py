...
for fold, (train_index, test_index) in enumerate(kf.split(x_2d_snv_train), start=1):
    ...
    pd.DataFrame(x_train).to_csv(f"{data_dir}x_train_fold{fold}", index=False)
    pd.DataFrame(x_test).to_csv(f"{data_dir}x_test_fold{fold}", index=False)
    pd.DataFrame(y_brix_train_fold).to_csv(f"{data_dir}y_brix_train_fold{fold}", index=False)
    pd.DataFrame(y_brix_test_fold).to_csv(f"{data_dir}y_brix_test_fold{fold}", index=False)
    pd.DataFrame(y_firm_train_fold).to_csv(f"{data_dir}y_firm_train_fold{fold}", index=False)
    pd.DataFrame(y_firm_test_fold).to_csv(f"{data_dir}y_firm_test_fold{fold}", index=False)
